...
class Controller:
    ...
    def cycle(self, dt: float) -> MachineState:
        ...
        # Запас по натяжению считается КАЖДЫЙ цикл, а не только при движении.
        # Он не команда, а свойство текущей позы, и смотреть на него нужно
        # именно до того, как разрешено движение: панель иначе показывала бы
        # ноль там, где на самом деле «ещё не считали», а ноль в этом поле
        # означает «платформу здесь не удержать».
        if pose is not None:
            self._equilibrium_tensions(pose)
